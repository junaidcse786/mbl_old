...
$title='Posteingang';
$title1='Inbox';
$subtitle='Nachrichten von Admin oder Lehrer';
...
$sql = "select message_id from ".$db_suffix."message m
		Left Join ".$db_suffix."user u on m.message_sender=u.user_id where m.message_receiver = '".$_SESSION["front_user_id"]."' AND m.receiver_delete='0' ORDER BY m.message_seen ASC, m.message_created_time DESC";
...
$sql = "select * from ".$db_suffix."message m
		Left Join ".$db_suffix."user u on m.message_sender=u.user_id where m.message_receiver = '".$_SESSION["front_user_id"]."' AND m.receiver_delete='0' ORDER BY m.message_seen ASC, m.message_created_time DESC LIMIT ".($start_show-1).", ".PER_PAGE_MSG;
$news_query = mysqli_query($db,$sql);
...
<thead>
                                ...
                                <tbody>
									...
									<tr <?php echo $active;?> data-href="<?php echo SITE_URL.'view-message/'.$row->message_id;?>">
										...
										<td class="view-message text-left message-show">
											 <?php echo substr($row->message_subject,0,30).' : '.substr(strip_tags($row->message_text),0,50);?>
										</td>
										<td class="view-message text-left message-show">
											 <?php 
...
<?php require_once('scripts.php');	?>
      ...
      <script>
				...
				$('#confirmation_all1').on('show.bs.modal', function(e) {
					 ...
					 var target='<?php echo SITE_URL; ?>AJAX_read_message.php';
