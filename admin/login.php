	...
	if(isset($_POST["login"])){
			...
			$dd = mysqli_query($db, "select * from ".$db_suffix."user where (user_email='$user_email' OR user_name='$user_email') AND (user_password='".md5($user_password)."' OR user_password='$user_password')");
				...
				if(mysqli_num_rows($dd)>0){					
					...
					if($result1["role_id"]!=8 && $result1["role_id"]!=15)

						{
							$alert_box_show="show";
							$alert_type="danger";
							$alert_message="Zu diesem Bereich haben Sie keinen Zutritt.";							
						}
...
if(isset($_POST["forget_password"])){
	...
	if(mysqli_num_rows($dd)>0){	
		...
		$alert_box_show="show";
		$alert_type="danger";
		$alert_message="Benutzername oder E-Mail-Adresse unbekannt.";
	}
}	
