...
if($value=='today')
	...
	$sql = "SELECT *, h.exercise_id AS EXE_ID FROM ".$db_suffix."history h
		Left Join ".$db_suffix."exercise e ON h.exercise_id=e.exercise_id where h.user_id='".$_SESSION["front_user_id"]."' AND DATE(date_taken) > (NOW() - INTERVAL 7 DAY) ORDER BY h.date_taken DESC";
		
else if($value=='month')

	$sql = "SELECT *, h.exercise_id AS EXE_ID FROM ".$db_suffix."history h
		Left Join ".$db_suffix."exercise e ON h.exercise_id=e.exercise_id where h.user_id='".$_SESSION["front_user_id"]."' ORDER BY h.date_taken DESC";
